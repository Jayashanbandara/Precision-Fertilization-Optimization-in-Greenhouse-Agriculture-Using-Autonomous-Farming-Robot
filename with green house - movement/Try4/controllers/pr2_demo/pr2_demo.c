...
#define MIN_RANGE 10  // minimum range to detect an obstacle
#define NUM_SAMPLES 100 // number of consecutive samples to average
...
static void enable_devices() {
  ...
  wb_camera_has_recognition(wide_stereo_l_stereo_camera_sensor);
  wb_camera_has_recognition(wide_stereo_r_stereo_camera_sensor);
  
  wb_camera_recognition_enable(wide_stereo_l_stereo_camera_sensor, 64);
  wb_camera_recognition_enable(wide_stereo_r_stereo_camera_sensor, 64);
...
//bool wb_lidar_is_point_cloud_enabled(WbDeviceTag tag);
  ...
  while (true) {
    ...
    // lift the arms
    set_left_arm_position(0.5, 1.0, 0.0, -0.08, 1, true); //(shoulder_roll, shoulder_lift,upper_arm_roll,elbow_lift, wrist_roll, wait_on_feedback)
    set_right_arm_position(-0.5, 1.0, 0.0, -0.08, 1, true); ////
    
    robot_go_forward(0.35);
    
    set_left_arm_position(0.07, 1.0, 0.0, -0.08, 1, true); //(shoulder_roll, shoulder_lift,upper_arm_roll,elbow_lift, wrist_roll, wait_on_feedback)
    set_right_arm_position(-0.07, 1.0, 0.0, -0.08, 1, true); ////
    
    set_left_arm_position(0.01, 1.0, 0.0, -1, 1, true); //(shoulder_roll, shoulder_lift,upper_arm_roll,elbow_lift, wrist_roll, wait_on_feedback)
    set_right_arm_position(0.01, 1.0, 0.0, -1, 1, true); ////
    
    robot_go_forward(0.7);
    robot_rotate(M_PI/2);
    robot_go_forward(2);
    ...
    robot_go_forward(1.6);
    robot_rotate(-M_PI/2);
    robot_go_forward(200);
    robot_rotate(-M_PI/2);
    break;
    // go to the other table
    //robot_go_forward(-0.35);
    //const double *robot_pos = wb_robot_get_position();
    //set_gripper(true, true , 50.0, true);   
    //set_gripper(false, true, 50.0, true);   //grabbing   (left/right,rotate,torque,wait_on_feedback)
...
////////////////recognition
    ...
    //wb_camera_get_image(wide_stereo_l_stereo_camera_sensor);
    //wb_camera_get_image(wide_stereo_r_stereo_camera_sensor);
